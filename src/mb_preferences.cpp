...
namespace mb_preferences
{
	...
	namespace str_
	{
		constexpr GUID guid_server = { 0x8732cde8, 0xe1ed, 0x4824,{ 0xae, 0xf6, 0x7d, 0x4b, 0x56, 0x99, 0x23, 0xac } };
		constexpr const char* default_server = "https://musicbrainz.org";
		cfg_string server(guid_server, default_server);

		constexpr GUID guid_albumtype = { 0x20968c09, 0xb0d4, 0x4059,{ 0xb8, 0x92, 0xda, 0x76, 0xf8, 0xe6, 0x51, 0x4e } };
		constexpr const char* default_albumtype = "MUSICBRAINZ_ALBUMTYPE";
		cfg_string albumtype(guid_albumtype, default_albumtype);

		constexpr GUID guid_albumstatus = { 0x77182aac, 0x1caa, 0x4793,{ 0xb7, 0x15, 0xcc, 0xf8, 0x97, 0xba, 0x11, 0x1a } };
		constexpr const char* default_albumstatus = "MUSICBRAINZ_ALBUMSTATUS";
...
public:
	...
	BOOL OnInitDialog(CWindow, LPARAM)
	{
		...
		server_str = GetDlgItem(IDC_EDIT_SERVER);
		albumtype_str = GetDlgItem(IDC_EDIT_ALBUMTYPE);
		albumstatus_str = GetDlgItem(IDC_EDIT_ALBUMSTATUS);
		...
		server_str.EnableWindow(mb_preferences::bool_::server.get_value());
		albumtype_str.EnableWindow(mb_preferences::bool_::albumtype.get_value());
		albumstatus_str.EnableWindow(mb_preferences::bool_::albumstatus.get_value());

		uSetWindowText(server_str, mb_preferences::str_::server);
		uSetWindowText(albumtype_str, mb_preferences::str_::albumtype);
		uSetWindowText(albumstatus_str, mb_preferences::str_::albumstatus);
	...
	bool has_changed()
	{
		...
		str8 temp;
		uGetWindowText(server_str, temp);
		if (mb_preferences::str_::server != temp) return true;
		uGetWindowText(albumtype_str, temp);
		if (mb_preferences::str_::albumtype != temp) return true;
		uGetWindowText(albumstatus_str, temp);
	...
	void apply() override
	{
		...
		uGetWindowText(server_str, mb_preferences::str_::server);
		uGetWindowText(albumtype_str, mb_preferences::str_::albumtype);
		uGetWindowText(albumstatus_str, mb_preferences::str_::albumstatus);
	}

	void on_change()
	{
		server_str.EnableWindow(server_checkbox.IsChecked());
		albumtype_str.EnableWindow(write_albumtype_checkbox.IsChecked());
		albumstatus_str.EnableWindow(write_albumstatus_checkbox.IsChecked());
	...
	void reset() override
	{
		...
		server_str.EnableWindow(mb_preferences::bool_::default_server);
		albumtype_str.EnableWindow(mb_preferences::bool_::default_albumtype);
		albumstatus_str.EnableWindow(mb_preferences::bool_::default_albumstatus);

		uSetWindowText(server_str, mb_preferences::str_::default_server);
		uSetWindowText(albumtype_str, mb_preferences::str_::default_albumtype);
		uSetWindowText(albumstatus_str, mb_preferences::str_::default_albumstatus);
